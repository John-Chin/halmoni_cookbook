...
const defaultState = {
  ...
  pantry: {
    ...
    pantry: [
      { id: "pantry-rice", text: "Rice" },
      { id: "pantry-noodles", text: "Dry noodles" },
      { id: "pantry-soy-sauce", text: "Soy sauce" },
    ],
  },
  recipes: [
    {
...
function normalizeState(input) {
  ...
  if (input.pantry && typeof input.pantry === "object") {
    ...
    if (Array.isArray(input.pantry.freezer)) {
      safeState.pantry.freezer = input.pantry.freezer.map(
        function (item, index) {
          return {
            id:
              typeof item?.id === "string" && item.id
    ...
    if (Array.isArray(input.pantry.pantry)) {
      safeState.pantry.pantry = input.pantry.pantry.map(function (item, index) {
        return {
          id:
            typeof item?.id === "string" && item.id
              ? item.id
